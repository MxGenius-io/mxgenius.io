...
- 2026-09-17: Built the first Customer Operations vertical slice in Operations Center. The new tenant-scoped workspace creates and edits customer companies, attaches any number of registered Pi devices, exposes unassigned hardware for reassignment, records an operational payment ledger without storing payment credentials, and summarizes heartbeat, desired-state, Equipment Drive, deployment, and error telemetry. Device Registry and Equipment Drives now live in that customer context instead of separate Settings cards, while preserving the existing registration, claim approval, publish, and assign contracts. An additive migration and manager/administrator APIs back the workspace; production deployment and signed-in acceptance remain pending.
