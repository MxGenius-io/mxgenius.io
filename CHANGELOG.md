...
- 2026-09-15: Added a non-destructive demo presentation mode for Maintenance and Parts. Once the tenant's explicit fictional seed is detected or refreshed, stale test and operational records no longer leak into case recall, inventory, shortages, requests, locations, component registers, import history, or report displays. The newest fictional maintenance case opens automatically, demo stock retains its labeled image treatment, mixed operational exports stay disabled while the presentation filter is active, and Settings provides a deliberate "Show all records" escape hatch. Corrected the demo maintenance records to satisfy the model-facing case contract and canonical aircraft identity, retained the friendly `N350MX` label in the interface, and removed the invalid facility count from the demo loader status. The underlying operational records are preserved.
