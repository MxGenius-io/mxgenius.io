...
- 2026-09-18: Expanded the fictional Parts presentation with 47 additional synthetic inventory cards across lighting, wheels and brakes, tires, electrical, avionics, filters, hydraulics, flight controls, cabin, sensors, consumables, and safety. Each seeded part has a matching tenant-derived stock unit and a distinct locally generated 1536×1024 catalog visual, resolved through a stable metadata key that never touches production stock. Demo mode now shows the complete fictional inventory instead of narrowing to the six Friday-flow parts and requests a bounded 200-row presentation window so the expanded catalog cannot be silently truncated. A frozen roster and automated one-to-one count, path, dimension, size, unique-hash, and multi-tenant seed checks keep future changes from drifting.

- 2026-09-18: Added **R&D Highlights** as the first Operations Center tab. The gallery collects all seven videos referenced across Weeks 1, 7, 17, 19, and 22, pauses competing players, links every recording back to its source report, and streams the real Git LFS media through the same byte-range-capable GitHub media path used by report rendering rather than the small LFS pointer files emitted by a normal Pages checkout. The environment manifest now exposes the collection to the model and contextual guidance layer.

- 2026-09-17: Added an organization-owned **JetNet Connection** Settings card and completed its service path. Managers and administrators can verify and replace their company JetNet account without exposing it to browser storage; the core stores AES-256-GCM ciphertext bound to the organization and provider, returns only a masked identity/status, and supplies the decrypted credential only to the fleet service over its internal authenticated lane. Fleet provider sessions and aircraft snapshots are now tenant-scoped, and a successful connection change immediately invalidates that tenant's old provider session. Restart Tour and Sign Out remain standalone and now share one responsive bottom row.

- 2026-09-17: Finished the Settings and Customer Operations information-architecture pass. Customer Operations now keeps Devices and Equipment Drives visible even before the first customer exists, then scopes approval and assignment to the selected account with explicit account labels. Settings now has one **AI** card for capabilities, model, cache, demo content, and ingestion; one **Customize** card for appearance and Azure-backed sounds; and standalone Restart Tour and Sign Out actions at the bottom.

- 2026-09-17: Reconnected Settings sound replacements to the live WebXR runtime. Embedded VR now inherits the signed-in dashboard's Azure-backed sound library instead of silently falling back to bundled cues, and saving or restoring a cue broadcasts a targeted invalidation so an already-open XR scene reloads only the changed audio on its next play. Cache versions were advanced across the dashboard, 3D viewer, and globe; the web-delivered fix does not require rebuilding the native Quest thermal companion.

- 2026-09-17: Deployed the first Customer Operations vertical slice in Operations Center. The new tenant-scoped workspace creates and edits customer companies, attaches any number of registered Pi devices, exposes unassigned hardware for reassignment, records an operational payment ledger without storing payment credentials, and summarizes heartbeat, desired-state, Equipment Drive, deployment, and error telemetry. Device Registry and Equipment Drives now live in that customer context instead of separate Settings cards, while preserving the existing registration, claim approval, publish, and assign contracts. Additive migration 0031 and manager/administrator APIs back the workspace. Production revision `mxg-core--cust22ac710`, all health surfaces, the authenticated Customers tab, and the existing managed device/unassigned queue were live-verified; mutation acceptance remains reserved for real customer data.

- 2026-09-15: Promoted both protected Rocky identities (`rocky@mxgenius.io` and `hagy2392@gmail.com`) to the MXGenius Administrator role. Added an additive production migration for existing memberships and made protected administrator seeds authoritative so a previous Procurement or Manager membership cannot survive a release and continue producing misleading `ACCESS_DENIED` results.

- 2026-09-15: Finished the clean Maintenance and Parts presentation pass. Settings now uses one **Show/Hide Demo Content** toggle: demo mode shows only the fictional presentation dataset and off mode shows only operational records, without deleting either set. The narrow Parts detail drawer stays pinned to the viewport with its own scroll region, and demo parts now use distinct generated visuals for wheels, brakes, tires, wheel hardware, filters, electrical components, pitot probes, landing lights, flight controls, and hydraulics instead of repeating a few generic images. Added a landing-light maintenance visual for the new-case flow. Registered CL350 manual figures are now returned to the browser whenever an image was supplied to the model—even for a conversational response—and an explicit CL350 image request takes precedence over an unrelated active-case aircraft model without changing normal case-scoped retrieval. Both response layouts constrain the figure and expose a clear unavailable state. The newest fictional maintenance case still opens automatically, identifiers remain human-readable, and all operational records remain preserved.

- 2026-09-14: Closed the maintenance-case intake and media recall release. The create action now gives focused inline validation instead of failing silently; cases display stable `MXG-CASE-YYYYMMDD-XXXXXXXX` references and real timestamps; operator-facing Equipment Pack wording is now Equipment Drive; and stored case images render through the existing tenant-authenticated application route. The Blob container remains private and users need no Azure Storage role—the service identity performs the read after the organization-scoped case lookup. Production revision `mxg-core--casemedia1` passed health, readiness, adapter, ordinary-user image recall, naming, and no-write validation checks.

- 2026-09-14: Reworked copilot awareness and conversational output. Structured chat now receives a server-owned application map plus request-scoped facts that distinguish authenticated core access, mounted capabilities, and downstream health; the blanket ban on acknowledging connectivity is replaced with attributed, evidence-based status language. Ordinary conversation uses a compact response with no maintenance advisory payload, while technical requests retain the strict cited advisory shape. Persisted history now stores natural assistant text instead of raw structured JSON, Realtime routing has explicit stock and spatial exceptions, and the header readiness badge is backed by a bounded core readiness probe rather than sign-in alone.

- 2026-09-07: Added a separate Migration lane to the shared Integration Readiness guide for moving personal or provisional control under the MXGenius entity. Eight folded starter rows cover Azure ownership and second-operator access, the later production-service/data cutover, Apple Developer and App Store Connect, Partner Center authentication, GitHub release continuity, domain/DNS/email administration, Meta Horizon/Quest publishing, and a final vendor-account sweep. Each row distinguishes platform requirements from MXGenius's internal safeguards, records current and target ownership, dependency order, next action, completion proof, rollback notes, and an official help link; unresolved names and ownership remain visibly marked for confirmation.

- 2026-09-04: Softened the new Integration Readiness workspace into a progressive-disclosure guide. The opening view is now a compact three-step orientation with small counts; software opens as a scannable list while devices, processes, current capabilities, enclosure context, structured-output guidance, and every detailed form stay folded until someone asks for them. Row summaries show only category or location, timing, owner, and readiness.

- 2026-09-04: Added an authenticated Integration Readiness workspace beside the Build Board and patent workspace. It turns the MXGenius stand-up into three editable lists: required software connections with purpose and exchange boundaries; internal and external black-box hardware with ports, power, readings, calibration, and demo proof; and five starter aviation response processes with evidence order, human authority, success criteria, and gold-standard examples. The shared document uses the existing tenant-scoped, versioned workspace boundary and starts with known Pi, DeWalt, FLIR, Quest, FAA, Boeing, Teams, ADP, PartSpace, JetNet, and internal-system prompts without treating unresolved details as facts.

- 2026-09-03: Connected Quest passthrough still capture to the active maintenance case instead of treating it as ephemeral model context. The sensor scene now restores the signed-in case, presents a compact evidence tray, saves a bounded JPEG through the existing confirmed observation and private Blob path, refreshes the case version, and animates the captured thumbnail into the tray. Recalled cases and the dashboard card consume the same tenant-scoped gallery; image and MP4/WebM evidence share authenticated playback routes with byte-range support. The spatial thermal display now reveals and retracts smoothly, while raw bridge traces remain available in the browser but no longer occupy the VR presentation. Native Quest video recording and the customer Remote Witness room/invite remain explicit follow-on work.
