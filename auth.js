...
(() => {
  ...
  const isDashboard = /(?:^|\/)(dashboard|progress|patent-workspace|build-board|feedback|feedback-admin)\.html$/i.test(location.pathname);
  ...
  function protectedReturnUrl() {
    ...
    try {
      ...
      if (!/(?:^|\/)(progress|patent-workspace|build-board|feedback|feedback-admin)\.html$/i.test(target.pathname)) return null;
      return target.href;
    } catch {
      return null;
    }
