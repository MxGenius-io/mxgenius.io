...
(() => {
  ...
  const isDashboard = /(?:^|\/)(dashboard|progress|patent-workspace|build-board|feedback|feedback-admin|integration-readiness)\.html$/i.test(location.pathname);
  ...
  function protectedReturnUrl() {
    ...
    try {
      ...
      if (!/(?:^|\/)(progress|patent-workspace|build-board|feedback|feedback-admin|integration-readiness)\.html$/i.test(target.pathname)) return null;
      return target.href;
    } catch {
      return null;
    }
