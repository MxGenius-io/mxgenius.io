...
test('Quest companion config uses the current Alpha candidate build identity', () => {
  assert.match(runtimeConfig, /sensorCompanionVersion: '0\.1\.0-poc\.16'/);
  ...
  assert.equal(metaRelease.uploadedBuild.versionCode, 16);
  assert.equal(metaRelease.uploadedBuild.versionName, '0.1.0-poc.16');
  assert.equal(metaRelease.uploadedBuild.status, 'UploadedPendingMetaVerification');
  assert.equal(metaRelease.build.versionCode, 16);
  assert.equal(metaRelease.build.versionName, '0.1.0-poc.16');
  ...
  assert.match(companionGradle, /versionCode = 16/);
  assert.match(companionGradle, /versionName = "0\.1\.0-poc\.16"/);
...
test('Quest FLIR companion is standalone and has no Pi runtime dependency', () => {
  ...
  assert.match(companionCamera, /activity\.runOnUiThread/);
