...
test('Quest companion config matches the published Alpha build', () => {
  ...
  assert.equal(metaRelease.build.versionCode, 24);
  assert.equal(metaRelease.build.versionName, '0.1.0-alpha.24');
  ...
  assert.match(companionGradle, /versionCode = 24/);
  assert.match(companionGradle, /versionName = "0\.1\.0-alpha\.24"/);
