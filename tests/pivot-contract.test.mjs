...
test('Quest companion config matches the published Alpha build', () => {
  ...
  assert.equal(metaRelease.build.versionCode, 25);
  assert.equal(metaRelease.build.versionName, '0.1.0-alpha.25');
  assert.match(metaRelease.build.metaTestStatus, /^LocalVerification(?:Pending|Passed)$/);
  ...
  assert.match(companionGradle, /versionCode = 25/);
  assert.match(companionGradle, /versionName = "0\.1\.0-alpha\.25"/);
