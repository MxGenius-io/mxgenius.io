...
async fn chat(
    ...
    for attempt in 0..MAX_CHAT_MODEL_ROUNDS {
        if attempt + 1 == MAX_CHAT_MODEL_ROUNDS {
            ...
            request_body["tool_choice"] = json!("none");
        }
        let upstream = match state
            .realtime_client
            .post(OPENAI_RESPONSES_URL)
...
mod structured_advisory_tests {
    ...
    #[test]
    fn chat_reserves_enough_rounds_for_grounded_inventory_synthesis() {
        assert_eq!(MAX_CHAT_MODEL_ROUNDS, 6);
        assert!(CHAT_SYSTEM_INSTRUCTIONS.contains(
            "Once the required manual and inventory results are returned, synthesize the answer immediately"
        ));
        assert!(CHAT_SYSTEM_INSTRUCTIONS.contains(
            "Do not spend a tool call resolving an aircraft again when the active case already supplies"
        ));
        assert!(CHAT_SYSTEM_INSTRUCTIONS.contains(
            "For description_query, use the shortest distinguishing catalog noun phrase"
        ));
    }
