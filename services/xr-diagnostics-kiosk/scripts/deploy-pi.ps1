...
$archive = Join-Path ([System.IO.Path]::GetTempPath()) "mxg-diagnostics-kiosk-release-$deploymentId.tgz"
$remoteArchive = "/tmp/mxg-diagnostics-kiosk-release-$deploymentId.tgz"
$remoteStage = "/tmp/mxg-diagnostics-kiosk-release-$deploymentId"
...
exit 1
'@
  $remoteInstall = $remoteInstall.Replace('__REMOTE_ARCHIVE__', $remoteArchive).Replace('__REMOTE_STAGE__', $remoteStage)
