...
`0.1.0-alpha.21` (`versionCode 21`) closes the native wearer-control surface. The immersive panel displays the single-use 7-digit guest PIN, intended audience, viewer count, expiry, approved layers, recording and network state, and explicit errors without rendering diagnostic logs. START performs wearer approval and fresh Horizon capture consent; PAUSE removes media, RESUME requires fresh consent, and END revokes the room. The panel reports LIVE only from the connected WebRTC peer, while credentials remain memory-only and absent from the rendered state.

`0.1.0-alpha.22` (`versionCode 22`) completes the consented guest-audio return path. The public witness page adds its user-enabled microphone track to the existing peer connection, while the Quest peer negotiates receive-only audio, enables device playout, and reports when the customer microphone track arrives. The wearer still owns room approval and screen capture; guest microphone and recording consent remain separate, explicit controls.

`0.1.0-alpha.23` (`versionCode 23`) makes that return path observable and reliable on Quest. The app owns a temporary voice-communication audio route and focus while a witness peer is active, restores the prior route on teardown, surfaces native playout failures, and reports customer audio as live only after both native playout and inbound RTP packets are present. The Quest remains receive-only and does not request wearer microphone permission.

`0.1.0-alpha.24` (`versionCode 24`) closes the merged WebXR Remote Witness launch gap. The canonical maintenance scene hands the wearer's explicit approve or resume action to the installed Quest companion, which opens Horizon's MediaProjection consent prompt for the already bootstrapped room and returns to the browser afterward. The companion's browser handoff now targets the unified maintenance workspace rather than the retired sensor-only scene.

`0.1.0-alpha.25` (`versionCode 25`) restores Remote Witness continuity across the Quest consent handoff. Android capture is constrained to the full headset display so returning from the companion cannot strand a single-app projection on a vanished activity. Browser and native media health now require advancing frames and RTP bytes, retain the last good frame during bounded recovery, and stop retrying after a finite budget. WebXR exits deliberately for consent and resumes VR or AR only from a fresh wearer gesture.
